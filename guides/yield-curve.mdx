...
description: "Query daily treasury yield rates, build yield curves, and track the 10Y-2Y spread with the MacroData API."
icon: "chart-line"
...
curl -H "X-API-Key: $MACRODATA_API_KEY" \
  "https://api.macrodata.dev/v1/treasury/yields?maturity=10Y&from=2026-01-01&to=2026-03-24"
```
...
resp = requests.get(
    "https://api.macrodata.dev/v1/treasury/yields",
...
```bash
curl -H "X-API-Key: $MACRODATA_API_KEY" \
  "https://api.macrodata.dev/v1/treasury/yields/curve?date=2026-03-24"
```
...
```bash
curl -H "X-API-Key: $MACRODATA_API_KEY" \
  "https://api.macrodata.dev/v1/treasury/yields/spread?long=10Y&short=2Y&from=2025-01-01"
```
...
```bash
curl -H "X-API-Key: $MACRODATA_API_KEY" \
  "https://api.macrodata.dev/v1/treasury/yields?maturity=10Y&curve_type=TIPS&limit=5"
```
