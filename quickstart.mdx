---
title: "Quickstart"
description: "Make your first MacroData API call in under 2 minutes. Get real economic data with one curl command."
icon: "rocket"
---
...
<Steps>
  <Step title="Get your API key">
    ...
    Your key starts with `mda_live_` — store it in an environment variable.

    ```bash
    export MACRODATA_API_KEY=mda_live_sk_YOUR_KEY_HERE
    ```
  </Step>
  <Step title="Fetch some data">
    ...
    curl -H "X-API-Key: $MACRODATA_API_KEY" \
      "https://api.macrodata.dev/v1/series/CPIAUCSL/observations?limit=3"
    ```
    ...
    resp = requests.get(
        "https://api.macrodata.dev/v1/series/CPIAUCSL/observations",
  ...
  <Step title="Try a point-in-time query">
    This is what makes MacroData different. Add `?as_of=2023-10-26` to see GDP as it was known the day the advance estimate dropped:

    <CodeGroup>
    ```bash cURL
    curl -H "X-API-Key: $MACRODATA_API_KEY" \
      "https://api.macrodata.dev/v1/series/GDPC1/observations?as_of=2023-10-26&limit=3"
    ```

    ```python Python
    resp = requests.get(
        "https://api.macrodata.dev/v1/series/GDPC1/observations",
        headers={"X-API-Key": os.environ["MACRODATA_API_KEY"]},
